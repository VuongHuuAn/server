...
sellerRouter.get("/seller/analytics", seller, async (req, res) => {
  try {
    console.log('Starting analytics request for seller:', req.user);
    ...
    let totalEarnings = 0;
    let categoryData = {
      Mobiles: { earning: 0, quantity: 0 },
      Essentials: { earning: 0, quantity: 0 },
      Appliances: { earning: 0, quantity: 0 },
      Books: { earning: 0, quantity: 0 },
      Fashion: { earning: 0, quantity: 0 },
    };
    ...
    orders.forEach((order) => {
      ...
      order.products.forEach((item) => {
        ...
        console.log('Product sellerId:', item.product.sellerId);
        ...
        if (item.product.sellerId.toString() === req.user) {
          ...
          console.log('For category:', item.product.category);

          if (categoryData.hasOwnProperty(item.product.category)) {
            categoryData[item.product.category].earning += earning;
            categoryData[item.product.category].quantity += item.quantity;
          }
        }
      });
    });
    ...
    const sales = Object.entries(categoryData)
      .map(([category, data]) => ({
        category,
        earning: data.earning,
        quantity: data.quantity
      }))
      .filter(item => item.earning > 0 || item.quantity > 0);
